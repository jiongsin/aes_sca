...
if { $version != "sca" } {
    echo "ERROR: aes_ctr_${version}.v is not expected in this flow. Use version=sca."
    exit 1
}
...
if {[file exists $saif_path]} {
    ...
    read_saif -input $saif_path -instance_name aes_ctr_tb/dut
} else {
    set saif_path_alt "./verif/sim/${run_name}/${run_name}.saif"

    if {[file exists $saif_path_alt]} {
        echo "Reading SAIF: ${saif_path_alt}"
        read_saif -input $saif_path_alt -instance_name aes_ctr_tb/dut
    } else {
